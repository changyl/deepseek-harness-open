...
describe('web e2e: shipped right Sidebar', () => {
  ...
  describe('with a settled session', () => {
    ...
    it('opens content once, splits, and floats it outside the column', async () => {
      ...
      // The chip names the change its Turn applied, so the tab opens on the
      // write's declared diff; the whole file — read over the same wire — is one
      // toggle away.
      const changeView = column.locator('[data-textpreview-change]')
      await changeView.waitFor({ timeout: 15_000 })
      expect(await changeView.innerText()).toContain('produced by the seeded turn')
      await column.locator('[data-textpreview-tool="change"]').click()
    ...
    it('keeps each session\'s surface to itself, and restores it on return', async () => {
      ...
      try {
        ...
        // The chip names the change its Turn applied, so the tab opens on the
        // diff; this case is about the file body and its panes, so it steps
        ...
        // The restored tab opens on its change again, so the file body is one
        // toggle away here too.
        if (await column.locator('[data-textpreview-change]').count() > 0) {
          await column.locator('[data-textpreview-tool="change"]').click()
        }
        expect(await column.locator('pre').first().innerText()).toContain('produced by the seeded turn')
