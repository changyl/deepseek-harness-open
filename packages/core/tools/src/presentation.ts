...
export interface FileDiff {
  ...
  newText: string
  /**
   * 1-based line in the file's new text where this hunk's `newText` begins.
   * A producer that diffed two full texts records it, which lets a viewer place
   * the change inside the whole file; a call-time intended diff omits it, because
   * a presenter that has not run the call cannot know the resulting line.
   */
  newStart?: number
}
