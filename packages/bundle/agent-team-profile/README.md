---
description: "Agent Teams profile layer over dsh-base with Team-scoped coordination tools, teammate delegation, and fresh workflow children."
kind: "package-bundle"
---
...
- [Bundle group](../README.md) — the bundle package map this layer joins.
- [Agent Teams service](../../subagent/agent-team/README.md) — durable roster, messaging, and task-board behavior.
- [Agent Teams tools](../../subagent/tool-agent-team/README.md) — the Team-scoped model tool surface.
...
The Team policy and schemas belong to [`@deepseek-ai/dsh-tool-agent-team`](../../subagent/tool-agent-team/README.md). This bundle changes composition only: Team-scoped `list_agents`, `send_message`, and `interrupt_agent` replace the disabled global continuable-child controls. `spawn_teammate` is the direct delegation tool. Workflow's `agent()` calls create fresh one-shot children; their prompts must contain the context needed for their tasks.

#### Token effect

The bundle adds the Team policy and tool schemas described by `@deepseek-ai/dsh-tool-agent-team`; it adds no prompt text of its own.
...
- **Workflow child tools** — the [Team tool visibility limitation](../../subagent/tool-agent-team/README.md#known-limitations-and-deferred-work) also applies to workflow children.
