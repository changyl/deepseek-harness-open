...
关闭时先保存未完成的清理请求并释放标签页，再在后台等待 Host 清理。失败时提供重试通知。每个请求使用独立的终端 ID localStorage key，清理成功或收到明确的 Session 域拒绝——`session/not-found`、`session/writer-held`、`session/agent-busy`，均在控制器运行前抛出——后删除，因为本实例没有能结束该进程的窗口；启动时重试已保存的请求。传输失败时保留请求。清理请求独立于标签关联和侧栏布局持久化。浏览器存储不可用时，内存中的清理仍可工作，但刷新后无法恢复该请求。
