 ...
 * applied change plus {@link DIFF_CONTEXT} context lines and the 1-based line its new side
 * starts on. Pure insertions use `oldText: null`, patch-only no-newline markers are omitted,
 * and scattered replacements remain separate hunks.
 *
 * `newStart` positions `newText` inside the after text, so a viewer holding that text can
 * draw the change in place among the file's unchanged lines.
 *
 ...
 * @returns one diff per applied hunk, in file order, each naming its line in the after text;
 *   empty when the texts are identical.
 */
export function computeHunkDiffs(path: string, before: string, after: string): (FileDiff & { newStart: number })[] {
  const patch = structuredPatch('', '', before, after, undefined, undefined, { context: DIFF_CONTEXT })
  const diffs: (FileDiff & { newStart: number })[] = []
...
function isFileDiff(value: unknown): value is FileDiff {
  ...
  const { path, oldText, newText, newStart } = value as Record<string, unknown>
  return typeof path === 'string'
    && (oldText === null || typeof oldText === 'string')
    && typeof newText === 'string'
    // Absent in metadata written before hunks carried a position, and in a
    // call-time intended diff, which has no resulting line to name.
    && (newStart === undefined || (typeof newStart === 'number' && Number.isInteger(newStart) && newStart >= 0))
}
