...
describe('computeHunkDiffs', () => {
  ...
  it('a scattered replace_all yields one FileDiff PER hunk (matching per-site editor blocks)', () => {
    ...
    // Each hunk names the line its new side starts on, so a viewer holding the
    // after text can place the change among the file's unchanged lines.
  ...
  it('a pure deletion of the whole file reports newText empty', () => {
    ...
    expect(diffs).toEqual([{ path: 'f.txt', oldText: 'gone', newText: '', newStart: 1 }])
  })
  ...
  it('drops the "\\ No newline at end of file" marker from a no-trailing-newline change', () => {
    ...
    expect(diffs).toEqual([{ path: 'f.txt', oldText: 'x', newText: 'y', newStart: 1 }])
  ...
  it('uses DIFF_CONTEXT (3) surrounding lines', () => {
    ...
    expect(diff?.oldText?.split('\n')[0]).toBe('line7')
    // The third context line before the change is line 7 of the after text.
...
describe('diffsFromMeta (defensive narrowing)', () => {
  ...
  it('accepts a hunk that names the line its new side starts on, and rejects a malformed one', () => {
    const meta = { diffs: [{ path: 'f.txt', oldText: 'a', newText: 'b', newStart: 7 }] }
    expect(diffsFromMeta(m(meta))).toEqual(meta.diffs)
    expect(diffsFromMeta(m({ diffs: [{ path: 'f.txt', oldText: 'a', newText: 'b', newStart: '7' }] }))).toBeUndefined()
    expect(diffsFromMeta(m({ diffs: [{ path: 'f.txt', oldText: 'a', newText: 'b', newStart: 1.5 }] }))).toBeUndefined()
    expect(diffsFromMeta(m({ diffs: [{ path: 'f.txt', oldText: 'a', newText: 'b', newStart: -1 }] }))).toBeUndefined()
  })
