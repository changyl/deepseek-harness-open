---
description: "Run a small team of named agents in one session: durable messages between members and a shared task board."
...
`dsh-agent-team` turns one coding session into a small working team: the session's agent becomes the Lead, creates named teammates for delegated work, exchanges durable messages with them, and tracks shared tasks on a common board. Messages and task state survive crashes, reloads, and interruptions, so a teammate that was offline receives its queued messages when it resumes. It provides no tools of its own — mount the sibling `dsh-tool-agent-team` so the model can create teammates, message them, and use the task board. It needs durable session storage to activate.
...
- [Agent Teams publication decision](../../../.agents/notes/implemented/architecture/2026-08-18-experimental-agent-teams-packages.md) — the superseded decision that published these packages from the experimental group.
- [Package promotion record](../../../.agents/notes/implemented/architecture/2026-09-16-agent-team-package-promotion.md) — why they are now ordinary release members in product-role groups.
