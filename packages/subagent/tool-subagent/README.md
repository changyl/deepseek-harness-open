...
| `agentCatalog` | `false` | Publish the durable available-subagent catalog while `ctx.agentDefinitions` is mounted and this instance's tool registration is the visible one |
| `catalogDescriptionMaxLength` | `500` | Maximum normalized definition-description length rendered per catalog entry; an integer of at least 3 |
...
### Selecting a specialized agent definition

The tool exposes an optional `agent_type` parameter only while the composition mounts `ctx.agentDefinitions` and the delegation provider advertises the `persona` capability; a call that names a definition without either fails before a child exists. The value is a definition name from the available-subagents catalog.

A selected definition overrides configured child policy for that call. Its persona body replaces `persona`, and its `tools` allow-list replaces `toolFilter`. Its `model` and `reasoning_effort` sit between the call's own `provider`, `model`, and `reasoning_effort` arguments and the configured `agentOptions`, and require enabled model selection plus a route the Session authorizes. Its `max_depth` only tightens the configured `maxDepth`, so the effective cap is the smaller of the two. A definition field that is absent leaves the configured behavior unchanged.

Every rejection happens before a child exists: an unknown or unavailable definition, a definition naming a tool the spawning agent cannot see, a definition whose `tools` or `max_depth` the provider cannot honor, and a definition route while model selection is disabled. The definition name is not persisted in the child descriptor.
...
| [`src/agent-catalog.ts`](src/agent-catalog.ts) | Durable available-subagent catalog text, message identity, and publication decisions |
...
The generated default [`subagent` schema](../../../docs/tool-catalog.md#deepseek-aidsh-tool-subagent) under this instance's configured name while its provider exists. An enabled Session policy adds `provider`, `model`, and `reasoning_effort` plus inheritance and selection guidance; the provider must support `agentOptions`. Provider context inheritance changes the tool and prompt descriptions. Enabled background mode adds `run_in_background`: continuable mode documents its `true` default, runtime settlement notice, and explicit foreground override, while one-shot mode documents its `false` default and the job id collected with `job_output` or stopped with `job_kill`. While the tool is visible in an assembly's scope, a `tool:<toolName>` system-prompt section tells the model to start independent continuable delegations together, keep working while they run, and choose foreground only when its next action depends on the result; a tool restriction removes both its schema and this guidance. An available-definition composition adds `agent_type` while `ctx.agentDefinitions` is mounted and the provider advertises `persona`.

#### Token effect

Fixed schema cost per parent request; model selection adds three parameters, and an available-definition composition adds `agent_type`. Each provider instance adds one schema, and each continuable instance adds one short system-prompt section.
...
With `agentCatalog: true`, the composition publishes one durable `plugin`-sourced user message identified as `tool-subagent/agent-catalog` before the first request, while `ctx.agentDefinitions` is mounted, at least one definition exists, and this instance's tool registration is the visible one. A later step publishes a replacement list when the winning membership or a rendered description changes, retires the list to `- (none)` after the last definition disappears, and leaves the last published list in place while discovery is incomplete. The rendered text does not depend on the publishing instance, so several capable delegation tools publish one list; descriptions are whitespace-normalized, truncated to `catalogDescriptionMaxLength` characters, and escaped for `<`, `>`, and `&`.
...
One short fixed user message per Session while the catalog is published; each definition adds one line whose description is capped at `catalogDescriptionMaxLength` characters.

#### KV Cache effect

Append-only: an unchanged list is never republished, and a changed list appends one message after the reusable prefix rather than invalidating earlier entries.
...
- **Configured child policy is fixed per instance** — `persona`, `toolFilter`, `maxDepth`, and `agentOptions` come from the mounted instance's configuration, and a definition selected through `agent_type` can only narrow them for one call. A composition without a mounted `ctx.agentDefinitions`, or a provider without the `persona` capability, cannot select a definition at all. LLM selection requires an enabled per-Session preference and a provider that advertises `agentOptions`; both in-process providers and DSH SDK advertise it, while ACP, Codex, and Claude Code reject it rather than ignore it.
