---
description: "Nine tools that let the model create, message, and coordinate teammates."
...
Add this package on top of `@deepseek-ai/dsh-agent-team` when the model should run a team through tools. Once mounted, every team member — the Lead and each teammate — gets the same nine tools plus the same coordination policy, stated in a paragraph that names its own role and name; `spawn_teammate` prefixes the initial task with the teammate's role and name.
