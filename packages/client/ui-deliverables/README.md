...
This package renders the changed-files card a finished turn ends with — the files the turn changed, with the Host's line counts, each opening the turn's review tab on that file and keeping review and whole-file reading in one tab — plus a keep-or-revert row over the turn's own mutations, cards for explicitly delivered files, and links matching inline-code references in the closing prose so a mentioned file opens in the right Sidebar. Listed and linked paths come from the recorded summary, successful mutations, and explicit deliveries, never from the prose, so a produced file is listed even unnamed. Only the shipped Web patch loads this package; removing its cordis.yml entry removes the guidance, cards, links, and change view together.
...
### The keep-or-revert row

The paths the turn's own mutations produced also render as one compact row: a chip per path, six shown before a remainder counter, each opening that file's preview on its change. The row's controls decide the whole turn — Accept keeps every change, Revert restores each file's content from before its change — and post one authenticated `change.review` request naming the changes by `tool/result` sequence and path only. Every byte written is re-derived from the session log, never supplied by the client, and a revert is verified against the file before anything is written, so a hunk whose text is no longer where the change put it fails the whole request instead of rewriting the wrong lines. Both decisions are durable session events, so a reload or a fork reconstructs them.
