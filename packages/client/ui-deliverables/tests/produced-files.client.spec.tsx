...
import {
  ...
  selectProducedChanges, selectProducedFiles, selectProducedReviews,
...
describe('produced-file Turn data', () => {
  ...
  it('records the applied hunks a successful mutation reports', () => {
    ...
    expect(changesForClosing(deliverablesOf(value))).toEqual([
  ...
  it('answers one change list per Turn while its produced array does not move', () => {
    ...
    expect(changesForClosing(data)).toBe(changesForClosing(data))
  })
  ...
  it('keeps the last change per path in first-seen order and drops hunk-less changes', () => {
    ...
    expect(changesForClosing(data, 5)).toEqual([
  ...
  it('derives a tail page whose Turn start fell outside the window, and keeps one entry when the start arrives', () => {
    ...
    expect(changesForClosing(deliverablesOf(value))).toHaveLength(1)
...
describe('presented files', () => {
  ...
  it('opens a delivered file on this Turn\'s change when it applied one', () => {
    ...
    const view = render(<Deliverables {...props} openFile={openFile} sessionId={SessionId('session')} t={makeTranslate(en)}
      matched={{ turn: 1, produced: [], changes: [{ path: 'report.docx', seq: 4, diffs }], presented: [{ path: 'report.docx', seq: 2, index: 0 }] }} />)
  ...
  it('opens a delivered file on its last recorded change when its Turn applied none', () => {
    ...
    const view = render(<Deliverables {...props} openFile={openFile} sessionId={SessionId('session')} t={makeTranslate(en)}
      matched={{ turn: 1, produced: [], changes: [], presented: [{ path: 'report.docx', seq: 2, index: 0 }] }} />)
    fireEvent.click(view.getByRole('button', { name: 'Open report.docx in sidebar' }))
...
it('shows descriptions and falls back to file metadata without hiding extensionless deliveries', () => {
  const view = render(<Deliverables {...openProps()} matched={{ turn: 1, produced: [], changes: [], presented: [
...
it('marks delivery cards that directly follow the produced-files row', () => {
  ...
  const view = render(<Deliverables {...shared} matched={{ turn: 1, produced: ['source.ts'], changes: [], presented }} />)
  expect(view.getByText('Files changed')).toBeTruthy()
...
it.each(['opening', 'opened', 'error'] as const)('shows the %s state and permits retries after failure', (phase) => {
  ...
  const view = render(<Deliverables {...props} matched={{ turn: 1, produced: [], changes: [], presented: [
...
it('explains a missing desktop and retries failed Host metadata', () => {
  const controller = new PresentedOpenController()
  const props = openProps(controller)
  const matched = { turn: 1, produced: [], changes: [], presented: [{ path: 'file.txt', seq: 2, index: 0 }] }
...
it('decides the whole Turn from the row, reporting the host reason when it refuses', async () => {
  ...
  const view = render(<Deliverables {...props} matched={{ turn: 1, produced: ['src/app.ts', 'src/util.ts'], changes, presented: [] }}
    openFile={() => {}} sessionId={SessionId('session')} t={makeTranslate(en)} />)
...
it('publishes its Turn changes and withdraws them when the row unmounts', () => {
  ...
  const view = render(
    <Deliverables
      {...props}
      matched={{ turn: 1, produced: ['src/app.ts'], changes, presented: [] }}
      openFile={() => {}}
...
it('loads desktop information only when delivery cards appear', () => {
  ...
  view.rerender(<Deliverables {...shared} matched={{ turn: 1, produced: [], changes: [], presented: [{ path: 'report.txt', seq: 2, index: 0 }] }} />)
  expect(props.reloadPresentedHost).toHaveBeenCalledOnce()
})
