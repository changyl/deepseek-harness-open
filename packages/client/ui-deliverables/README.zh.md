...
本包渲染已完成轮次末尾的改动文件卡片——列出本轮改动的文件及 Host 记录的行数，每一行在该文件上打开本轮的 review tab，让「查看改动」与「阅读完整文件」留在同一个 tab——以及覆盖本轮自身改动的一行「保留或回滚」控件、显式交付文件的卡片，并把收尾正文中匹配的行内代码引用转为链接，让被点名的文件在右侧 Sidebar 中打开。列出与链接的路径来自记录的改动摘要、成功的文件修改与显式交付，而非收尾正文，因此即使模型没有点名，产出文件也会被列出。只有正式提供的 Web patch 加载本包；删除其 cordis.yml 条目会同时移除指引、卡片、链接与改动视图。
...
本轮自身改动产出的路径还会渲染成一行紧凑的条目：每个路径一个 chip，显示六个后以计数收尾，点击即在对应改动上打开该文件的预览。该行的控件决定整轮结果——接受保留全部改动，回滚把每个文件恢复到其改动之前的内容——并以一次经认证的 `change.review` 请求提交，请求仅以 `tool/result` 序号和路径标记改动。写入的每个字节都从会话日志重新推导，绝不由客户端提供；回滚在写入任何内容之前先与文件核对，因此文本已不在改动落点的 hunk 会使整个请求失败，而不会改写错误的行。两种决定都是持久的会话事件，重新加载或派生会话都能重建它们。
