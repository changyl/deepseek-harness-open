...
/** Where in a file an open should land, and what the file is shown as. */
export interface OpenFileOptions {
  /** 1-based line to reveal; absent = the file's beginning. */
  readonly line?: number
  /**
   * `tool/result` seq of the Turn change the reader asked for. A preview that
   * can resolve it opens on the change instead of the whole file; one that
   * cannot shows the file, so an unresolvable seq is never an error.
   */
  readonly changeSeq?: number
  /**
   * 0-based change region inside that change to land on; absent = its first
   * one. Stepping backwards into another file asks for that file's last region.
   */
