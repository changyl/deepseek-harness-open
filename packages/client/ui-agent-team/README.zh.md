...
本包向 Web 会话页头添加 Agent Teams action，让用户检查当前 roster、管理共享任务板并导航到 teammate 会话。它通过生成的 `ctx.remote.agentTeams` contribution 读取权威 Team 状态，并让普通 child history 导航继续使用稳定的 addressed-subagent 路径。通过 Agent Teams Web profile 选择本包。这个浏览器 projection 不扩展稳定 API Proxy、不存储 Team 状态，也不注册面向模型的输入。
...
本面板由随包发布的 [`dsh-web-app`](../../bundle/web-app/README.zh.md) 组合包挂载。可选的宿主侧 [`dsh-agent-team-profile`](../../bundle/agent-team-profile/README.zh.md) 层负责启用 Team 工具。Web Client loader 挂载 `/client` export；root Host export 不执行行为，本包也没有用户配置字段。
...
Client export 挂载来自 [`@deepseek-ai/dsh-agent-team/remote`](../../subagent/agent-team/README.zh.md) 的生成的 `ctx.remote.agentTeams` contribution，然后通过 Cordis effect 注册 locale dictionary 与一个 conversation-header slot。Dispose plugin fiber 会移除这两项 registration。
...
- [Web 应用组合包](../../bundle/web-app/README.zh.md)——挂载本 Client plugin 的随包发布组合包。
- [Agent Teams 宿主 profile](../../bundle/agent-team-profile/README.zh.md)——启用 Team 工具的可选宿主层。
...
- [Client plugin 组](../README.zh.md)——本 plugin 加入的 Client 包地图。
