...
This package adds an Agent Teams action to the Web conversation header, where a user can inspect the current roster, manage the shared task board, and navigate into a teammate's conversation. It reads authoritative Team state through the generated `ctx.remote.agentTeams` contribution and keeps ordinary child-history navigation on the stable addressed-subagent path. Choose it through the Agent Teams Web profile. The browser projection does not extend the stable API Proxy, store Team state, or register model-facing input.
...
This panel is mounted by the shipped [`dsh-web-app`](../../bundle/web-app/README.md) bundle. The opt-in Host-side [`dsh-agent-team-profile`](../../bundle/agent-team-profile/README.md) layer is what enables the Team tools. The Web Client loader mounts the `/client` export; the root Host export is inert, and the package has no user configuration fields.
...
- [Web app bundle](../../bundle/web-app/README.md) — the shipped bundle that mounts this Client plugin.
- [Agent Teams Host profile](../../bundle/agent-team-profile/README.md) — the opt-in Host layer that enables the Team tools.
...
- [Client plugin group](../README.md) — the Client package map this plugin joins.
