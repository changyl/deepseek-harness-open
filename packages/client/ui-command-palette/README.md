...
Rows are data only. `PaletteEntry` is a discriminated union — `new-session`, `stop`, and `command` (which carries the session id its catalog was read for) — and a pick dispatches on that discriminant: `ctx.uiWorkspace.startSession()`, the bound session's `cancel()`, or `ctx.commandUi.run(name, session)`. Workspace navigation is read per pick, never captured at registration: the palette waits on the command surface and the Session Controller, while the Workspace UI also needs its directory-picker Remote and can therefore register later. New Session focuses the composer of the session `startSession()` resolves with; a session created fresh instead mounts one whose unlock effect takes the caret. Contribution icons stay out of the row: a `ComponentType` is neither JSON-compatible nor a callback, and UI domains share only those, so the overlay draws one glyph per entry kind instead.
