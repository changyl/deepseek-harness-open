...
客户端命令贡献项或被装饰的宿主命令会打开自己的弹窗或运行自己的动作，与从菜单选择该行完全一致。其他宿主命令以裸命令行分离执行，因此 `/plan` 会进入计划模式，`/compact` 会立即压缩。命令面板不会向输入框插入任何内容，也不会从中删除任何内容——命令面板的选择不拥有任何命令 token，所以你输入的草稿会留在原处。
