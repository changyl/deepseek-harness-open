...
Double-click the terminal's tab title to rename it. **Take control** makes the current attachment writable when another page owns input. A temporary disconnect preserves the screen and offers **Reconnect**, without exposing transport diagnostics. A terminal that could not start offers **Retry** and **Close**; closing drops its connection and removes the tab. An exited shell remains visible with its exit code and offers **New terminal**; it never restarts automatically. Exited terminals count toward the Session limit; close unused tabs when the limit is reached.

Closing or replacing a terminal tab removes it immediately and ends its process in the background. A cleanup failure shows a small notification with **Retry**; retrying does not reopen the tab. Cleanup the Host refuses because the Session is gone, or because another harness instance holds it, retires its saved request instead of retrying it: no window here can end that process, and the Host reclaims a terminal no window holds. Collapsing, switching tabs or Sessions, floating and fullscreen presentation preserve the process.
