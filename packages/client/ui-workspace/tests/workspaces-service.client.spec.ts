...
describe('UiWorkspaceService', () => {
  ...
  it('uses only an explicit Workspace or the recent-Workspace policy for new Sessions', async () => {
    ...
    await expect(b.uiWorkspace.startSession(wid('recent-home'))).resolves.toBe(sid('created-recent-home'))
    expect(b.sessions.retain).toHaveBeenLastCalledWith(sid('created-recent-home'), { source: 'mainView' })
