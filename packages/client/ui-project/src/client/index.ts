/** Project-board global panel over the generated `project` Remote namespace. */

import type { Context as ClientContext } from '@deepseek-ai/cordis'
import type {} from '@deepseek-ai/dsh-client-locale/client'
import type {} from '@deepseek-ai/dsh-client-ui-renderer/client'
// Type-only: declares the `main` keyed slot this plugin occupies.
import type { MainPanelId } from '@deepseek-ai/dsh-client-ui-layout/client'
// Type-only: declares the `sidebar.panellist` row this plugin occupies.
import type {} from '@deepseek-ai/dsh-client-ui-sidebar/client'
// Type-only: loads the generated `project` namespace declaration onto `ctx.remote`.
import type {} from '@deepseek-ai/dsh-api-remotes/client'
import { ProjectPanel, type ProjectPanelInjected } from './ProjectPanel.tsx'
import { ProjectPanelIcon } from './ProjectPanelIcon.tsx'
import { en, NS, zh, type ProjectLocaleKey } from './locales.ts'

export type { ProjectPanelInjected, ProjectPanelProps } from './ProjectPanel.tsx'
export type { ProjectLocaleKey } from './locales.ts'

declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface LocaleNamespaceMap {
    /** Project-board panel copy. */
    projects: ProjectLocaleKey
  }
}

/** Services required by the panel registrations and the generated Remote face. */
export const inject = ['slots', 'locale', 'remote', 'remote.project']

/** Main-panel key shared by the sidebar row and the centre-column occupant. */
const PANEL_ID = 'project' as MainPanelId

/**
 * Client plugin body: register the dictionaries, the global panel, and its
 * sidebar row.
 * @param ctx - client root context.
 */
export function apply(ctx: ClientContext): void {
  ctx.effect(() => ctx.locale.register(NS, { zh, en }), 'ui-project: dictionaries')

  const t = ctx.locale.bind(NS)
  const list: ProjectPanelInjected['list'] = async () => {
    const result = await ctx.remote.project.list(undefined)
    if (!result.ok) {
      throw new Error(`project.list failed: ${result.error.code}: ${result.error.message}`)
    }
    return result.value
  }
  const board: ProjectPanelInjected['board'] = async (id) => {
    const result = await ctx.remote.project.board(id)
    if (!result.ok) {
      throw new Error(`project.board failed: ${result.error.code}: ${result.error.message}`)
    }
    return result.value
  }

  ctx.slots.inject('main', () => ctx.slots.register({
    name: 'main',
    key: PANEL_ID,
    locale: NS,
    inject: () => ({ list, board }),
  }, ProjectPanel))

  ctx.slots.inject('sidebar.panellist', () => ctx.slots.register({
    name: 'sidebar.panellist',
    id: PANEL_ID,
    order: 10,
    label: () => t('nav'),
  }, ProjectPanelIcon))
}
