---
description: "右侧 Sidebar 的文档预览与文本编辑：共享文件加载与控件，可选 Markdown、代码、图片、PDF 和 HTML 渲染器，以纯文本兜底，并为声明可编辑的渲染器提供编辑器。"
...
在右侧 Sidebar 预览可读文件，无需另开 tab 即可切换已注册的渲染器。Markdown 和代码接收累计文本页；PDF、HTML 和常见图片接收完整字节；未知文件扩展名使用纯文本。声明 `editable` 的渲染器还会开启编辑会话：草稿整文件读取，保存时携带其来源版本，被他人推进过的文件会报告为冲突而非被覆盖。tab 负责加载、文件状态、渲染器选择、换行、重新载入以及该会话；文档正文通过同一元数据注册表与子 slot 注册。Sidebar tab 的 kind 为 `text`。
