---
description: "右侧 Sidebar 的文档预览与文本编辑：共享文件加载与控件，可选 Markdown、代码、图片、PDF、Office 和 HTML 渲染器，以纯文本兜底，并为声明可编辑的渲染器提供编辑器。"
...
在右侧 Sidebar 预览可读文件，无需另开 tab 即可切换已注册的渲染器。Markdown 和代码接收累计文本页；PDF、HTML 和常见图片接收完整字节；未知文件扩展名使用纯文本。Office 文档在本地转换为 PDF。声明 `editable` 的渲染器还会开启编辑会话：草稿整文件读取，保存时携带其来源版本，被他人推进过的文件会报告为冲突而非被覆盖。tab 负责加载、文件状态、渲染器选择、换行、重新载入以及该会话；文档正文通过同一元数据注册表与子 slot 注册。Sidebar tab 的 kind 为 `text`。
...
- **正文** —— keyed slot `sidebar.right.pane.tab`，键为类型的 id。固定头部在可用时显示 Host 的绝对路径，否则显示请求路径；目录使用三级标签色，文件名使用一级标签色，路径过长时保留末段并向开头淡出，提示中仍提供完整值。有多个受支持的渲染器时才显示下拉菜单。仅文本兼容的源文件提供纯文本选项；只有一个渲染器时不显示查看器控件。已知的二进制容器后缀没有注册渲染器时，在路径头部下方显示文件类型图标和不支持预览的说明，并且不会发起读取。仅当所选渲染器声明 `wrap: true` 时显示换行开关；图标表示点击后切换到的模式，该偏好按 tab 保存，初始开启。当 tab 的 `file` 导航携带改动索引能回答的 `changeSeq` 时，此头部出现两个控件，正文改为打开该轮次改动而非文件本身。第一个在改动与完整文件之间切换；第二个把改动画成左「旧」右「新」的两栏对比——全屏面板的宽度正适合这种布局。两种布局都按该文件扩展名对应的语法（与代码渲染器同一套映射）绘制改动，因此改动的源码文件保留其语法配色；没有对应语法的扩展名保持纯文本正文。读取完成后，对比会连同文件自身的行一起展示，并在选中期间继续分页读取整份文件，因此改动出现在它在文件中的位置上；读取无法对应的 hunk 则只显示改动本身。「上一处 / 下一处」会连同位置（`3/17`）一起显示，用于遍历**本轮所有改动**的每一处，而不只是这个文件的：在文件的一端再步进会打开相邻改动（向后时落在上一个文件的最后一处），并在本轮的第一处与最后一处停止。两个布局控件在任意面板尺寸下都可用；导航到另一个改动会回到改动本身，并按读者所选布局绘制，该布局由会话保留，适用于每一处改动，也适用于步进为其他文件打开的 tab。未索引或不存在的 seq 让 tab 停在文件上，此时不显示该控件，而不是把它置灰。重新载入仍在此头部，不放入 Sidebar 的 tab 条。正文贴合格的每条边，各渲染器自行提供内容留白，并可拥有内部滚动区。这与 Files tab 右侧预留 2px 滚动条间距的布局有意不同：Preview 使用格的完整宽度，使贴边 HTML 与代码滚动区终止于格的边缘。
- **共享加载与视图状态**，会话作用域、按 tab id 分桶。store 持有累计页或完整字节、读取与观察版本、加载/失败状态、渲染器选择、滚动位置、换行、已响应的导航 revision，以及会话保留的单栏/两栏改动布局。普通 inject face 调用 Remote 读取，并经声明的 store action 写入。重新载入和加载模式变化会淘汰旧请求；tab 的中止信号清理其状态。

文档实现在 `ctx.documentPreviews.register({ id, extensions, binaryExtensions?, priority, title, loading, wrap? })` 注册元数据，并以相同 `id` 向 keyed、Session 作用域的子 slot `sidebar.right.tab.document` 注册正文。`binaryExtensions` 列出 `extensions` 中不可按文本阅读的后缀，这些后缀不提供纯文本选项。两处注册都由 effect 持有，通过 `ctx.slots.inject` 等待子 slot。正文接收 `resourceAddress`、准备好的 `content`、`wrap`、`scrollportRef` 和标准 `useTabInfo`/`useResource` 钩子，不接收自定义资源加载器。拥有内部滚动元素的渲染器把 `scrollportRef` 挂到该元素上；该元素卸载后，owner 恢复使用共享正文。元数据声明 `loading: 'text-pages'` 或 `'bytes-complete'`，并可声明 `editable: true`；编辑能力是声明的，而不是由加载模式推断，因为分页渲染器并不自动等于文本渲染器。注册表保留所有匹配备选：`extension`（默认）优先于 `builtin`，随后按更长的后缀、再按注册顺序排列。所选实现仍可用时，下拉选择保持不变；移除后选择下一个候选。内置正文也使用相同注册方式。HTML、SVG 和未匹配的扩展名保留纯文本回退，与加载方式无关。
...
- **编辑** —— 声明 `editable` 的渲染器会在头部提供一个 **编辑** 控件，规则见下文 [编辑](#editing)。
...
纯文本、Markdown 与代码渲染器声明 `editable: true`；完整字节渲染器不声明，因此该控件是不存在而不是被置灰。只有在 tab 展示完整文件、且分页读取已将该文件接受为文本时，才会为 Session 文件地址提供编辑。

进入编辑器会以第二种方式读取文件。`openDraft` 调用 `remote.workspaceFiles.readAll(sessionId, path, signal)`，并用 `TextDecoder('utf-8', { ignoreBOM: true })` 解码，因为一页是文件的有损视图：分页得到的 `text` 是其各行重新拼接的结果，因此以换行结尾的文件与不以换行结尾的文件得到完全相同的文本。以分页作为编辑器初值会在首次保存时剥掉那个换行。整文件读取让草稿保持字节忠实，包括开头的字节序标记与 CRLF 换行——两者都不会在任何环节被规范化。分页预览保有自己的读取路径与状态；草稿从不喂给它，它也从不喂给草稿。

会话是每个 tab 一个对象，持有草稿、用于比较的基线、草稿的来源版本，以及它的保存中、失败与冲突状态。草稿与基线不同即为「已修改」，**保存** 控件仅在这种情况下可用。保存调用 `remote.workspaceFiles.write(sessionId, path, { text, expectedVersion }, signal)`，携带草稿的来源版本。对于已被推进的文件，Host 以 `workspace-file/stale-version` 拒绝且不写入任何内容；编辑器将其报告为冲突，并恰好给出两种解决方式：**覆盖保存**（省略 `expectedVersion` 保存）与 **放弃并重新载入**（重新整文件读取并采用另一位写入者留下的内容）。其余每一种拒绝——`workspace-file/read-only`、`workspace-file/session-not-live`、后端失败——都作为普通失败报告，草稿与其基线保持不变。被接受的保存会把写入的文本设为新基线、把返回的版本设为下一次的守卫，并保持会话打开；它刻意不回写草稿，因此在保存进行期间输入的击键会让该 tab 重新变为已修改，而不是被丢弃。保存还会重新载入编辑器背后的预览，因为那份预览描述的仍是写入之前的文件。

面板为窄栏时，编辑器占据整列，预览被隐藏而非卸载，因此正文的滚动容器保留读者的位置。面板进入全屏后，两者以各占一半的固定比例共用该列，预览一侧会继续分页直到文件末尾。用于离开编辑器的预览控件在草稿已修改时处于禁用状态，因此离开不会静默丢弃草稿；**放弃修改** 才是明确表达该意图的控件。

编辑器的表面是朴素的 `<textarea>`：它不持有自己的文档模型，因此 store 始终是草稿的唯一来源。它只写 UTF-8；草稿存放在 tab 的 store 中，因此切换 tab 会保留，重新载入页面则不会。

<a id="navigation"></a>
## 导航

`ctx.sidebarRight.openResource(address, { params: { line, changeSeq } })` 通过 `file` 参数携带 1 起算的源码行号与某轮次改动的 `tool/result` seq。改动经 deliverables 插件提供的可选 `sessionFileChanges` 服务读取；没有它的组合对改动一律回答空，每个 tab 都停在文件上。在 `text-pages` 模式下，owner 顺序加载到该行或 EOF。纯文本与代码渲染器提供源码行锚点；Markdown 不提供。所选渲染器没有锚点时，导航保持待处理；用户切换到纯文本或代码后执行。代码导航直接滚动内部源码视口。字节模式渲染器不消费源码行导航。每个完成的导航 revision 只响应一次。不带 `revealIfOpened: false` 打开同一文件时聚焦已有 tab，并送达新 revision。
...
- **没有共享搜索接口。** 目录地址以 `not-regular-file` 失败；未知扩展名使用纯文本读取，仍受其 UTF-8/NUL 检查限制。
...
- **编辑限于整文件、纯文本与 UTF-8。** 只有声明 `editable` 的渲染器提供编辑；文件必须是分页读取已接受为文本的普通 Session 文件，且整份文件必须落在 Host 的 `maxFileBytes` 上限内。表面是朴素的 `<textarea>`：没有语法高亮、行号或多光标。其他编码的文件无法往返，因为浏览器写回的是 UTF-8。
- **草稿不持久。** 它存放在 tab 的 store 中，因此能跨 tab 切换与渲染器切换保留，但不能跨页面重新载入；关闭 tab 会在不询问的情况下丢弃它。没有自动保存：版本守卫与冲突提示都依赖读者自行选择写入时机。
