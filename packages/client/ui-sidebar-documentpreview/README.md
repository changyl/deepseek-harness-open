---
description: "Document previews and text editing in the right Sidebar: shared file loading and controls, selectable Markdown, code, image, PDF, Office and HTML renderers, plain-text fallback, and an editor for the renderers that declare themselves editable."
...
Preview readable files in the right Sidebar and choose among registered renderers without opening another tab. Markdown and code receive accumulated text pages; PDF, HTML, and common images receive complete bytes; unknown file extensions use plain text. Office documents convert locally to PDF. Renderers declaring `editable` add an editing session: the draft is read whole, saves carry the version it came from, and a file that moved on is reported as a conflict instead of overwritten. The tab owns loading, file status, renderer selection, wrap, reload, and that session; document bodies register through the same metadata registry and child slot. The Sidebar tab kind is `text`.
