/**
 * `sidebarDocumentPreview` namespace dictionaries.
 *
 * The failure lines are the point of this file: a preview that cannot show a
 * page has to say which of several different things went wrong, and each one
 * suggests a different next step for the reader.
 */

/** Simplified Chinese dictionary and key-set source of truth. */
export const zh = {
  loading: '正在读取…',
  loadMore: '加载更多',
  changed: '文件已更新，当前显示为旧内容。',
  reloadNow: '重新载入',
  reload: '重新读取文件',
  'wrap.enable': '自动换行',
  'wrap.disable': '取消换行',
  'wrap.aria': '自动换行',
  openWith: '打开方式',
  'viewer.text': '纯文本',
  resourceUnavailable: '文件资源服务不可用。',
  rendererUnavailable: '预览器 {name} 不可用。',
  'error.notFound': '文件不存在，可能已被移动或删除。',
  'error.tooLarge': '单页内容超过 {limit} 上限，无法读取。',
  'error.notText': '非文本文件，暂时无法预览。',
  'error.notRegularFile': '该路径不是普通文件，没有可显示的内容。',
  'error.readOnly': '当前会话为只读，无法保存该文件。',
  'error.sessionNotLive': '会话已不在运行，无法保存该文件。',
  'error.staleVersion': '文件在编辑期间被外部修改，未保存。',
  'error.writeFailed': '保存失败：{message}',
  'error.unavailable': '读取失败：{message}',
  retry: '重试',
  'edit.start': '编辑',
  'edit.startAria': '编辑此文件',
  'edit.preview': '预览',
  'edit.previewAria': '预览此文件',
  'edit.save': '保存',
  'edit.saving': '保存中…',
  'edit.dirty': '有未保存的修改',
  'edit.discard': '放弃修改',
  'edit.overwrite': '覆盖保存',
  'edit.reload': '放弃并重新载入',
  'edit.conflict': '文件在编辑期间被外部修改，请选择保留哪一份。',
  'edit.editorAria': '文件内容编辑器',
  'edit.splitAria': '编辑区与预览区',
  'change.show': '本次改动',
  'change.aria': '本次会话对该文件的改动',
  'change.toFile': '显示完整文件',
  'split.label': '横向对比',
  'split.aria': '并排对比本次改动',
  'split.show': '并排显示新老文件',
  'hunk.prev': '上一处',
  'hunk.next': '下一处',
  'hunk.position': '{index}/{count}',
  'review.accept': '接受此改动',
  'review.accepted': '已接受',
  'review.revert': '回滚此改动',
  'review.reverted': '已回滚',
  'review.pending': '处理中…',
  'review.failed': '操作失败',
  'diff.copy': '复制',
  'diff.copied': '已复制',
  'diff.collapseAria': '收起差异',
  'diff.expandAria': '展开其余 {count} 行差异',
  'diff.expandRest': '… 其余 {count} 行',
  'diff.collapse': '收起',
  'diff.files.one': '{count} 个文件',
  'diff.files.other': '{count} 个文件',
} satisfies Record<string, string>

/** Text-preview dictionary key union. */
export type SidebarDocumentPreviewKey = keyof typeof zh

/** English dictionary, checked against the Chinese key set. */
export const en = {
  loading: 'Reading…',
  loadMore: 'Load more',
  changed: 'The file has changed, showing the previous content.',
  reloadNow: 'Reload',
  reload: 'Read the file again',
  'wrap.enable': 'Turn on line wrap',
  'wrap.disable': 'Turn off line wrap',
  'wrap.aria': 'Line wrap',
  openWith: 'Open with',
  'viewer.text': 'Plain text',
  resourceUnavailable: 'The file resource service is unavailable.',
  rendererUnavailable: 'The {name} preview is unavailable.',
  'error.notFound': 'File not found. It may have been moved or deleted.',
  'error.tooLarge': 'This page exceeds the {limit} limit and cannot be read.',
  'error.notText': 'Not a text file, preview is unavailable for now.',
  'error.notRegularFile': 'Not a regular file, nothing to display.',
  'error.readOnly': 'This session is read-only, so the file cannot be saved.',
  'error.sessionNotLive': 'The session is no longer running, so the file cannot be saved.',
  'error.staleVersion': 'The file changed while you were editing it, so nothing was saved.',
  'error.writeFailed': 'Save failed: {message}',
  'error.unavailable': 'Read failed: {message}',
  retry: 'Retry',
  'edit.start': 'Edit',
  'edit.startAria': 'Edit this file',
  'edit.preview': 'Preview',
  'edit.previewAria': 'Preview this file',
  'edit.save': 'Save',
  'edit.saving': 'Saving…',
  'edit.dirty': 'Unsaved changes',
  'edit.discard': 'Discard changes',
  'edit.overwrite': 'Overwrite',
  'edit.reload': 'Discard and reload',
  'edit.conflict': 'The file changed while you were editing it. Choose which version to keep.',
  'edit.editorAria': 'File content editor',
  'edit.splitAria': 'Editor and preview',
  'change.show': 'This change',
  'change.aria': 'This session’s change to the file',
  'change.toFile': 'Show the whole file',
  'split.label': 'Side by side',
  'split.aria': 'Side-by-side comparison of this change',
  'split.show': 'Compare the old and new file side by side',
  'hunk.prev': 'Previous change',
  'hunk.next': 'Next change',
  'hunk.position': '{index}/{count}',
  'review.accept': 'Accept this change',
  'review.accepted': 'Accepted',
  'review.revert': 'Revert this change',
  'review.reverted': 'Reverted',
  'review.pending': 'Working…',
  'review.failed': 'Review failed',
  'diff.copy': 'Copy',
  'diff.copied': 'Copied',
  'diff.collapseAria': 'Collapse diff',
  'diff.expandAria': 'Expand {count} more diff lines',
  'diff.expandRest': '… {count} more lines',
  'diff.collapse': 'Collapse',
  'diff.files.one': '{count} file',
  'diff.files.other': '{count} files',
} satisfies Record<SidebarDocumentPreviewKey, string>
