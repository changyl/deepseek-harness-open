...
`packages/subagent/agent-team`、`packages/subagent/tool-agent-team`、`packages/bundle/agent-team-profile` 与 `packages/client/ui-agent-team` 是公开 workspace 包——发布名分别为 `@deepseek-ai/dsh-agent-team`、`@deepseek-ai/dsh-tool-agent-team`、`@deepseek-ai/dsh-agent-team-profile` 与 `@deepseek-ai/dsh-client-ui-agent-team`——并加入 dsh 发布系列。[发布拒绝列表决策](../process/2026-09-12-experimental-publication-denylist.zh.md)负责发布政策与其私有例外；[实验性包规则](../../../../packages/experimental/AGENTS.md)负责依赖隔离、本发布例外与后续 promotion。这里提到的浏览器层此后已退役：promotion 之后，它唯一的那条 `ui-agent-team` 行由随包发布的 [`dsh-web-app`](../../../../packages/bundle/web-app/README.zh.md) 组合包挂载。
