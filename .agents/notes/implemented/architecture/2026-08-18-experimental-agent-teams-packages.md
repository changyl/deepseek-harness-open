...
`packages/subagent/agent-team`, `packages/subagent/tool-agent-team`, `packages/bundle/agent-team-profile`, and `packages/client/ui-agent-team` are public workspace packages — `@deepseek-ai/dsh-agent-team`, `@deepseek-ai/dsh-tool-agent-team`, `@deepseek-ai/dsh-agent-team-profile`, and `@deepseek-ai/dsh-client-ui-agent-team` — and join the dsh release family. The [publication denylist decision](../process/2026-09-12-experimental-publication-denylist.md) owns the publication policy and its private exceptions; the [experimental package rules](../../../../packages/experimental/AGENTS.md) own dependency isolation, this publication exception, and later promotion. The browser layer named here has since retired: after promotion, its single `ui-agent-team` row is mounted by the shipped [`dsh-web-app`](../../../../packages/bundle/web-app/README.md) bundle.
...
The published Host-side Agent Teams profile bundle depends on the Team packages and applies after `dsh-base`. It inserts the Team rows and disables the global continuable-child controls whose model-visible names overlap the Team tools. The Team UI no longer ships as its own profile layer: the shipped `dsh-web-app` bundle mounts it after the Host profile, and it mounts the Remote contribution generated by the Team package. The Host layer remains opt-in and leaves the shipped base, CLI, Web, and Python runtime dependency graphs unchanged.
