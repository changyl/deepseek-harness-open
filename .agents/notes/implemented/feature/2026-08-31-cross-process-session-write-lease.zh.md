...
两个 harness 实例共用同一个会话根目录，是使用者遇到这一拒绝的常见方式，因此 API Session controller 把争用归类为 `session/agent-busy` 并携带可执行的 reason——关闭持有该会话的实例，或在进行中的打开落定后重试——而不是把它折成未归类的 `gateway/internal` 失败。这是调用方可以据以行动的状态，而非内部故障。
