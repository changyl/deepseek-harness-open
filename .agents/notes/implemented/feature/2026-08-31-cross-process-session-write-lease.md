...
Two harness instances sharing one session root is the ordinary way a person meets this refusal, so the API Session controller classifies contention as `session/agent-busy` carrying an actionable reason — close the instance that holds the session, or retry once an in-flight open settles — rather than folding it into an unclassified `gateway/internal` failure. The rejection is a state the caller can act on, not an internal fault.
